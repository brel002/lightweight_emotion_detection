...
def load_topk(csv_path: Path, top_k: int, metric_name=None, filter_gap=None):
    ...
    with csv_path.open(newline="", encoding="utf-8") as f:
        ...
        for row_index, r in enumerate(reader):
            ...
            # Phase-3 params
            lr= parse_float_safe(r.get("lr"))    
            wd= parse_float_safe(r.get("weight_decay")) 
            ls= parse_float_safe(r.get("label_smoothing"))
...
def main():
    ...
    ap.add_argument("--csv", default=None)                  # concatenated results from previous phases
    ...
    # get file with results from  previous ablation phases
    ...
    total = len(base_cfgs)
    print(f"[grid] Phase-3: {total} runs (from top-{len(base_cfgs)} Phase-test configs)")
    ok = 0; t0 = time.time()
    ...
    for base in base_cfgs:
        ...
        cmd = [
            ...
            "--ablation-phase=4",           # append to Phase-0,Phase-1,Phase-2 results file
            f"--seed={args.seed}",
